...
const EditEntryForm = () => {
    ...
    const { entries, fetchEntries } = useContext(MstContext);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const entryRef = await firestore.collection('entries').doc(params.id);

            await firestore.collection('entries').doc(entryRef.id)
                .set({
